...
import { FaChevronLeft, FaChevronRight, FaStore, FaHome, FaCar, FaUtensils, FaBriefcase, FaTshirt, FaMobileAlt, FaLaptop, FaGamepad, FaPills, FaGem, FaShoppingBag, FaHotel, FaGlassCheers, FaGasPump, FaTruck, FaHospital, FaClinicMedical, FaSprayCan, FaDumbbell, FaGraduationCap, FaBook, FaGift, FaMapMarkerAlt } from 'react-icons/fa';
import { useTheme } from 'next-themes';
import dynamic from 'next/dynamic';

const PostDetailsModal = dynamic(() => import('./PostDetailsModal'), {
  ssr: false
});
...
const categoryIcons = {
  ...
  'jewelry': { name: 'مجوهرات وذهب', icon: 'FaGem' },
  ...
  'lands': { name: 'أراضي', icon: 'FaMapMarkerAlt' },
  ...
  'entertainment': { name: 'أماكن ترفيهية', icon: 'FaGlassCheers' },
  ...
  'beauty': { name: 'مراكز تجميل', icon: 'FaSprayCan' },
...
const DynamicSection = ({ section }) => {
  ...
  const fetchItems = useCallback(async () => {
    try {
      ...
      // سجل معلومات القسم الحالي
      console.log('معلومات القسم الحالي:', {
        sectionId: section.id,
        sectionTitle: section.title,
        sectionName: section.name
      });
      
      const apiUrl = `/api/posts?category=${section.id}`;
      ...
      console.log('البيانات الخام المستلمة:', data);
      
      // استخراج العناصر من خاصية items مع قيمة افتراضية مصفوفة فارغة
      let itemsData = Array.isArray(data?.items) ? data.items : [];
      
      console.log('جميع العناصر المستلمة:', itemsData);
      
      // تصفية العناصر بناءً على فئة القسم الحالي
      ...
      console.log(`تم العثور على ${itemsData.length} عنصرًا بعد التصفية`);
      
      if (itemsData.length === 0) {
        console.warn('لم يتم العثور على عناصر مطابقة للفئة المطلوبة');
        
        // عرض جميع العناصر المتاحة للتحقق
        const allItems = Array.isArray(data?.items) ? data.items : [];
        const allCategories = [...new Set(allItems.map(item => item.category_name).filter(Boolean))];
        console.log('جميع الفئات المتاحة في البيانات:', allCategories);
        console.log('جميع الفئات المعرفة في التطبيق:', Object.values(categoryIcons).map(c => c.name));
      }
  ...
  const scrollTo = (direction) => {
    ...
    const { scrollLeft: currentScroll, clientWidth, scrollWidth } = containerRef.current;
        ...
        {(showLeftArrow || showRightArrow) && (
          <>
            <button 
              ...
              className={`absolute left-4 top-1/2 -translate-y-1/2 z-10 p-3 bg-white/80 dark:bg-gray-800/80 backdrop-blur-sm rounded-full shadow-lg hover:bg-white dark:hover:bg-gray-700 transition-all duration-300 transform hover:scale-110 ${!showLeftArrow ? 'opacity-0 group-hover:opacity-100' : 'opacity-100'}`}
            ...
            <button 
              ...
              className={`absolute right-4 top-1/2 -translate-y-1/2 z-10 p-3 bg-white/80 dark:bg-gray-800/80 backdrop-blur-sm rounded-full shadow-lg hover:bg-white dark:hover:bg-gray-700 transition-all duration-300 transform hover:scale-110 ${!showRightArrow ? 'opacity-0 group-hover:opacity-100' : 'opacity-100'}`}
            ...
            >
                ...
                {item.image ? (
                  ...
                  <div className="w-full h-full flex items-center justify-center bg-gradient-to-br from-amber-50 to-amber-100 dark:from-gray-700 dark:to-gray-800">
                    <span className="text-4xl text-amber-400">
                      {section.icon && React.createElement(getIconComponent(section.icon))}
                    </span>
                  </div>
                )}
                <div className="absolute bottom-3 left-3 bg-amber-500 text-white text-xs font-bold px-2 py-1 rounded-full">
                  {categoryIcons[section.id]?.name || section.title}
                </div>
              </div>
              ...
              <div className="p-5 flex flex-col flex-grow">
                ...
                <div className="flex items-center text-gray-600 dark:text-gray-300 text-sm mb-4">
                  ...
                  <span className="line-clamp-1 rtl:mr-1">
                    {item.governorate || item.location || 'غير محدد'}
                  </span>
                </div>
